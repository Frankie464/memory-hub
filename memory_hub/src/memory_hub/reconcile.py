...
PATTERNS = {
    ...
    "relationship": [
        (re.compile(r"\bmy (?:girlfriend|boyfriend|partner) ([A-Z][a-z]+)\b"), "Partner: {}"),
        (re.compile(r"\bmy roommate(?:s)? ([A-Z][a-z]+(?:,?\s+and\s+[A-Z][a-z]+)*)\b"), "Roommate(s): {}"),
        (re.compile(r"\bmy (?:sister|sibling) ([A-Z][a-z]+)\b"), "Sibling: {}"),
    ],
    "financial": [
...
def _generate_profile(db_path: Path = DB_PATH) -> None:
    """Write profile.generated.md from active facts. Never touches manual.md."""
    ...
    lines = [
        ...
        "# NOTE: For manual edits, use profile.manual.md",
        "",
    ]
